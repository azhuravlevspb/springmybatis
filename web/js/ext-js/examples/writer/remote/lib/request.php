 ...
 */
class Request {
    public $restful, $method, $com.springmybatis.controller, $action, $id, $params;
    ...
    protected function parseRequest() {
        ...
        if (isset($_SERVER["PATH_INFO"])){
            $cai = '/^\/([a-z]+\w)\/([a-z]+\w)\/([0-9]+)$/';  // /com.springmybatis.controller/action/id
            $ca =  '/^\/([a-z]+\w)\/([a-z]+)$/';              // /com.springmybatis.controller/action
            $ci = '/^\/([a-z]+\w)\/([0-9]+)$/';               // /com.springmybatis.controller/id
            $c =  '/^\/([a-z]+\w)$/';                             // /com.springmybatis.controller
            $i =  '/^\/([0-9]+)$/';                             // /id
            $matches = array();
            if (preg_match($cai, $_SERVER["PATH_INFO"], $matches)) {
                $this->com.springmybatis.controller = $matches[1];
